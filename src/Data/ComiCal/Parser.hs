...
parseReleaseNumber seriesSlug lastPath  =
    do let prefix = BS.unpack seriesSlug <> "-"
...
parseReleaseURI tags =
    do anchorTag <- listToMaybe (dropWhile (~/= ("<a>" :: String)) tags)
...
parseImageDate tags =
    do spanTag <- listToMaybe . filter isTagText $
...
parseDCDate =
    ...
    BS.unpack .
    fromAttrib "content" .
    head . dropWhile (~/= ("<span property=\"schema:datePublished\">" :: String))
